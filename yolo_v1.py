...
train_labels = sorted(glob(
    'BDD/bdd100k/labels/100k/train/*'))         #sorted排序，glob返回所有匹配的文件路径列表，只含文件名
...
def load_data(image_path, labels):
    flip = tf.cast(tf.random.uniform(
        shape=[1, ], minval=0, maxval=2, dtype=tf.int32), dtype=tf.bool).numpy()[0] #将随机产生的数组转化为int32
...
with strategy.scope():
    model = get_darknet_19(H, W, output_shape)          #生成骨干网络
    model.compile(loss=Yolo_Loss,
                  optimizer=tf.keras.optimizers.Adam(learning_rate=5e-4))   #选择损失函数以及SGD
    print(model.output)
